...
<ResponseField name="clientSecret" type="string" required>
  Payment client secret for completing the payment (Stripe integration)
</ResponseField>
...
<Note>
  The `clientSecret` should be used with your payment provider (Stripe) to complete the payment process. The booking will expire after 15 minutes if payment is not completed.
</Note>
